...
| 40 | `examples/other/40_motor_spin_duration_check.py` | Finds the real spin duration for one target turn angle, on the real track surface | `PYTHONPATH=src python3 examples/other/40_motor_spin_duration_check.py --speed 150 --duration-s 2.24 --direction right` | ⚠️ Operator beside it |
| 41 | `examples/other/41_motor_spin_angle_sweep.py` | Sweeps spin duration vs. actual turned angle (operator reports each angle by eye) to (re)fit `spin_rate_deg_per_s`/`spin_dead_time_s` | `PYTHONPATH=src python3 examples/other/41_motor_spin_angle_sweep.py --speed 150` | ⚠️ Operator beside it, reads and types each angle |
